...
// ── build layout ───────────────────────────────────────────────────────────
function buildLayout(income, expenses, width, height) {
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div ref={svgRef} style={{ background: "#16152a", borderRadius: 14, padding: "12px 8px", border: "1px solid #2d2b4e" }}>
            <svg width="100%" height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} style={{ overflow: "visible" }}>
              ...
              {nodes.map(n => {
                ...
                const right = n.col >= 3;
                const lx = right ? n.x + nodeWidth + 6 : n.x - 6;
                const anchor = right ? "start" : "end";
                const my = n.y + n.h / 2;
                const fs = Math.min(11, Math.max(7, n.h * 0.26));
                return (
                  <g key={n.id}>
                    <rect x={n.x} y={n.y} width={nodeWidth} height={n.h} fill={c} rx={3}
                    ...
                    <rect x={n.x - 6} y={n.y} width={nodeWidth + 12} height={n.h} fill="transparent"
          ...
          <div style={{
            ...
            padding: "8px 14px", border: "1px solid #2d2b4e", fontSize: 11, color: "#d4d4f7",
            display: "flex", alignItems: "center",
          }}>
            {hovLink ? (
              <span>
                <span style={{ color: "#6b6b8a", textTransform: "uppercase", fontSize: 9, letterSpacing: "0.1em" }}>Flow · </span>
              ...
              <span style={{ color: "#3d3d5c" }}>Hover a flow to see details</span>
            )}
          </div>
        </div>
